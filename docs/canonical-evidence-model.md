...
| `entry_hash` | hash | SHA-256 over the RFC 8785 canonical encoding of **every other field in this table**, `prev_event_hash` included — see "What the entry hash covers" below |
| `notes` | string, optional | Free text (e.g. reason for access) |
| `hash_algo` | string, nullable | Digest algorithm that produced `entry_hash` (e.g. `SHA-256`) |
| `sig_alg` | string, nullable | Signature algorithm over the entry (e.g. `ED25519`) |
| `key_id` | string, nullable | Version-pinned KMS key identity that produced `signature` (ADR-0009) |
| `preimage_version` | integer, nullable | Which **field set** went into the preimage — distinct from the encoding, which is versioned separately |
| `signature` | bytes, nullable | Signature over the canonical encoding of `(ledger, sequence_number, prev_event_hash, entry_hash)`, made under `KeyPurpose.EVIDENCE_ROOT` by a key the application's database role cannot read. Stored as a self-describing envelope carrying the algorithm, key id and version, so the entry stays verifiable after key rotation |
| `anchor_ref` | string, nullable | Reference to the external RFC-3161 anchor covering this entry's Merkle root |
...
**Why hash-chaining alone is not sufficient**, and what the last six fields are for: a hash chain is only tamper-*evident* against someone who cannot recompute it. A privileged writer — a hostile DBA, a compromised application, a malicious insider — can rewrite an entry and recompute every subsequent hash, producing a chain that verifies perfectly. PRD SR-4 requires tamper-evidence *"even to an administrator with direct database access"*, which a bare chain cannot deliver. **ADR-0003 supersedes the integrity approach this section previously implied**: entries become authenticated (signed under a key held in a KMS the database role cannot read), externally anchored (Merkle roots timestamped via RFC-3161 into WORM storage, making rollback and truncation detectable), and crypto-agile. The six fields are the agility metadata that keeps history verifiable across algorithm and key rotation over a 10–15 year retention horizon.

**They are nullable by design, and a null is meaningful.** The subsystem lands in dependency order (`docs/modernization-roadmap.md` Wave 1): the columns exist from Wave 1.1, Wave 1.2 populates the signing metadata at write time, and Wave 1.3 fills `anchor_ref` asynchronously once a root is timestamped. A null therefore records "written before that guarantee existed" — a fact the Verification Engine (Wave 1.4) dispatches on and reports honestly, rather than a missing value. Defaulting them to a plausible-looking algorithm name would assert that an entry was signed when no signature exists, which is precisely the kind of unearned claim a court-facing record must not make.

**What the entry hash covers.** Since the complete-preimage change (ADR-0003 §2), `entry_hash` is computed over *all* of the entry's persisted fields: `custody_event_id`, `evidence_id`, `sequence_number`, `event_type`, `occurred_at`, `actor_id`, `actor_role`, `authority_ref`, `notes`, `integrity_hash_at_event`, and `prev_event_hash`, plus `hash_algo` and `preimage_version`. Only four columns are excluded, each for a structural reason: `entry_hash` itself, and `signature`/`key_id`/`sig_alg`/`anchor_ref`, which are written *after* the hash exists (a signature covers the hash; an anchor covers a Merkle root built from many hashes).

This completeness is the point, not a detail. The earlier preimage covered six fields and omitted `actor_id`, `actor_role`, `authority_ref`, and `notes` — meaning who handled a piece of evidence, in what role, and under what legal authority could all be rewritten without breaking a single hash in the chain. On a chain-of-custody record those *are* the evidence. A ledger that binds the payload digest but not the custodian answers a question nobody asked.

**`hash_algo` and `preimage_version` are inside the hash, not merely beside it.** This is a downgrade defense: if the metadata that tells a verifier *how* to check an entry were only stored next to that entry, an attacker could rewrite the entry under the older, weaker format and reset the version so it verified under the weaker rules. Because both are covered, changing either changes the hash.

**Two timestamps that look alike are not.** The hashed form of `occurred_at` is the same RFC 3339 `Z` form the API puts on the wire, so a client hashes exactly the bytes it received. An implementation that hashes a locally re-formatted timestamp — `+00:00` instead of `Z`, or a millisecond-truncated value — will fail to reproduce the hash of a perfectly intact entry.

**Why the signature is what closes SR-4, and the hash alone is not.** A hash chain is tamper-*evident* only against someone who cannot recompute it, and nothing about a hash requires a secret. An administrator with direct database access can edit an entry, recompute its `entry_hash`, recompute every subsequent one, and produce a chain that verifies perfectly. The signature cannot be recomputed that way: it needs the private key, which lives in the KMS/HSM. So a verifier must check **both** — the hash proves the entry's contents are the ones that were hashed, the signature proves the platform is what hashed them. Confirming the hash alone would report a forged chain as intact.

**And what the signature still does not cover: removal.** An insider who deletes the last entries of a chain, or restores an older backup, leaves a shorter ledger in which every remaining entry verifies. Nothing inside the database can detect that, because the evidence of the missing entries is precisely what was removed. Only an externally-anchored monotonic root (ADR-0003 §3) makes truncation and rollback detectable. Until that exists, "verified" means *this ledger is authentic and unaltered*, never *this ledger is complete*.

**Verifying across the format change.** Entries written before the complete preimage existed carry `preimage_version = NULL`. They were hashed over the partial field set with a non-canonical encoder and **cannot be re-derived** by a current verifier. A verifier must dispatch on `preimage_version` and report such an entry as *not independently verifiable*, never as failed: the chain links and sequence are still checkable for those entries, and calling an old-format entry "tampered" is a false accusation on a legal surface. The same rule protects an older client meeting a newer version — degrade, do not accuse.

**Canonical encoding.** Every hash in this section is computed over **RFC 8785 (JCS)** canonical JSON, implemented in `apps/server/src/sentinelai/platform/crypto/canonical.py`. This is a correctness requirement, not a formatting preference: custody and audit payloads live in PostgreSQL `jsonb`, which discards key order and normalizes numbers, so a preimage built with an ordinary JSON encoder changes when the same row is read back — and the entry would fail its own verification years later with nothing having been tampered with. JCS pins key ordering (by UTF-16 code unit), number formatting (ECMAScript `Number::toString`), and string escaping (minimal, literal UTF-8), so an independent verifier — including one not written in Python — reproduces the identical bytes. The encoding is versioned independently of `preimage_version`, so the field set and the serialization can each evolve without invalidating the other.
