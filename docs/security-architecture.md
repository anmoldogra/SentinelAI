...
1. **Hash chain, signature, and anchor re-verification (data tampering) — BUILT (Wave 1.4, ADR-0003 §6):** a scheduled job periodically re-verifies the custody-event and audit-log chains, **verifies each entry's signature against the KMS public key**, and **recomputes each published Merkle root against what the ledger holds now**, alerting immediately on any break — proactive detection, not just "verify on read." The three checks catch different attackers, and none is redundant: the hash catches an edit that left the digest stale; the signature catches an edit whose digest was recomputed (nothing about a hash requires a secret, so hashing alone does not survive an attacker with write access); the anchor catches a deleted **tail**, which the other two structurally cannot — remove the last N entries and every survivor still links, still hashes, still verifies. An entry that carries no signature at all (written before ADR-0003 §1 landed) is reported as *not independently verifiable* rather than as failed — on a court-facing report those are different findings, and only one of them is honest.

    Implemented as one engine with no I/O (`platform/crypto/verification.py`) behind two callers, so the on-demand report and the scheduled sweep cannot drift: `GET /api/v1/evidence/{id}/verify` (`api-design.md` §5.1) and an hourly worker cron job. **A fourth check applies where configured:** an anchor may carry an RFC 3161 timestamp token, verified against deployment-configured TSA trust anchors (`platform/crypto/tsa.py`, Wave 1.3c). That closes *backdating* — an attacker holding both the application and the clock republishing a doctored history as old. It is off by default and **refused outright in air-gapped/classified profiles**, which cannot reach a third party by definition and therefore retain that narrow residual deliberately; no deployment gets certificate revocation checking, for the same connectivity reason (ADR-0003's 2026-09-25c amendment records both limits). **Alarm channel:** Prometheus metrics (`sentinelai_ledger_verification_state`, `..._findings_total`) plus a `CRITICAL` structured log line, per §49's monitoring — deliberately not the notification module, which cannot resolve a recipient for a finding addressed to security operations rather than to any user (ADR-0003's 2026-09-25 amendment records why).
...
|---|---|---|
| Hash chain + signature + anchor re-verification | Data tampering (evidence, audit log), including tail truncation and restore-from-backup | Immediate alert (§49) via metric + `CRITICAL` log; affected records quarantined pending investigation. A restore of an evidentiary database is itself an integrity event requiring re-verification before return to service (`deployment-architecture.md` Part 7) |
